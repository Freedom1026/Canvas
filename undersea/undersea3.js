...
BlackFish[0].src = '../GUI/Bfish0.png';
BlackFish[1] = new Image();
BlackFish[1].src = '../GUI/Bfish1.png';
...
function timeAdd() {
    ...
    if (timer < 0) {
        ...
        GameOverCoin = coin;
        if (GameOverCoin < 20) {
            localStorage.setItem('rank_3', "1stars");
        }
        else if (GameOverCoin < 40) {
            localStorage.setItem('rank_3', "2stars");
        }
        else { localStorage.setItem('rank_3', "3stars"); }
...
function Pro2(x, y, dx, dy, sqm) {
    ...
    this.draw2 = function () {
        ...
        ctx.drawImage(BlackFish[this.sqm], this.x, this.y, 100, 100);
    }
    ...
    this.change2 = function () {
        ...
        this.y += this.dy;
        this.sqm += 1;
        if (this.sqm >= BlackFish.length) { this.sqm = 0; }
        this.draw2();
    }
}
...
function create2() {
    for (var i = 0; i < 40; i++) {
        ...
        FishArray2.push(new Pro2(x, y, dx, dy, sqm));
    }
}
...
function move() {
    ...
    for (let i = 0; i < FishArray2.length; i++) {
        FishArray2[i].change2();
    }
    moneyPro();
    if (FishArray.length < 20) {
            create();
        }
    else if (FishArray2.length <20){
        create2();
    }
    //清除掉超出螢幕的魚
    for (let i = 0; i < FishArray.length; i++) {
